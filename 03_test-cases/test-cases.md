...
| TC-028 | Sorting selection persists after product navigation | Logged in, Inventory | 1) Set sort (e.g., Price (low to high))<br>2) Open product details<br>3) Back to products | Sorting remains selected | Positive | Low | Fail | #2 | - |
...
| TC-033 | Checkout validation: spaces only | Checkout Step One | 1) Enter only spaces in First/Last/Zip<br>2) Continue | Validation error shown | Negative | High | Fail| #3 | - |
| TC-034 | Checkout robustness: very long inputs | Checkout Step One | 1) Enter 200+ chars in First/Last<br>2) Continue | App doesn’t break UI/crash; shows validation or accepts safely | Negative | Medium | - | - | - |
